...
library(dplyr)
w2 = w %>% select(client_FID, agency_program_FID, s_total.intake, s_total.discharge, everything())

df.s = subset(w2, s_total.intake != "NA" & s_total.discharge != "NA", )


attach(df.s)

df.s$agencyID[df.s$agency_FID.intake == 2] <- 10010
df.s$agencyID[df.s$agency_FID.intake == 3] <- 10011
df.s$agencyID[df.s$agency_FID.intake == 11] <- 10024
df.s$agencyID[df.s$agency_FID.intake == 12] <- 10034
df.s$agencyID[df.s$agency_FID.intake == 24] <- 10096

df.s$program[agency_program_FID == 154 | agency_program_FID == 155 | agency_program_FID == 156 | agency_program_FID == 157 | agency_program_FID == 158 | agency_program_FID == 159 | agency_program_FID == 209] <- "Day Treatment"

df.s$program[agency_program_FID == 	8	 |
...
df.s$program[agency_program_FID == 	1	 |
...
df.s$program[agency_program_FID == 	99 | agency_program_FID == 	112 | agency_program_FID == 	152 | agency_program_FID == 	185] <- "Independent Living"

df.s$program[agency_program_FID == 	3	 |
...
df.s$program[agency_program_FID == 	2	 |
...
df.s$program_loop <- paste(df.s$agencyID, df.s$program)


df.s$s_total.intake = as.numeric(as.character(df.s$s_total.intake))
df.s$s_total.discharge = as.numeric(as.character(df.s$s_total.discharge))
