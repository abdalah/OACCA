...
\begin{itemize}
\item `r length(belowagg$r8_total.intake)` Aggregate records available
\end{itemize}
...
if(length(belowagg$r8_total.intake) > 0){
  ...
  print(xtable(tab.1, align="lrrr", caption = "Caregiver Strength Attainment: A Comparison of Scores at \n Intake and Discharge for Caregivers of Children Ages Birth - 4", digits=c(0, 0, 2, 2)), type='latex', comment=FALSE, include.rownames=FALSE, include.colnames = F, caption.placement = "top", booktabs = TRUE, add.to.row = addtorow)
...
\begin{itemize}
\item `r length(aboveagg$r8_total.intake)` Aggregate records available
\end{itemize}

```{r, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
if(length(aboveagg$r8_total.intake) > 0){
  ...
  print(xtable(tab.1, align="lrrr", caption = "Caregiver Strength Attainment: A Comparison of Scores at Intake and Discharge for Caregivers of Children Ages 5+", digits=c(0,0, 2, 2)), type='latex', comment=FALSE, include.rownames=FALSE, include.colnames = F, caption.placement = "top", booktabs = TRUE, add.to.row = addtorow)} else{cat("Note: No useable records are available to generate the table and figure.")}
```


```{r, echo=FALSE, fig.align='center', fig.path= "Graphics/plot"}
if(length(aboveagg$r8_total.intake) > 0){
  plot(tab.1[, 2:3], type="b", ylim=c(0, 20), ylab="", xaxt="no", xlab="", col="red", main="Figure 2: Caregiver Strength Attainment: A Comparison of Scores at \n Intake and Discharge for Caregivers of Children Ages 5+", cex.main=0.8, cex.axis = .8, font.main = 1, family="serif")
  par(new = T)
  axis(1, at=1:2, labels=c("Intake", "Discharge"), col.axis="black", las=1, cex.axis = .75)
}
```
