...
prog <- df.s[ df.s$program_loop == prog, ]
...
\item `r length(prog$s_total.intake)` Agency records available
\end{itemize}
...
cans5b.iA = mean(prog$s_total.intake)
cans5b.dA = mean(prog$s_total.discharge)
...
print(xtable(tab.1, align="lrrrrrr", caption = "Education Outcomes: A Comparison of Scores at Intake and Discharge", digits=c(0, 0, 2, 2, 0, 2, 2)), type='latex', comment=FALSE, include.rownames=FALSE, include.colnames = F, caption.placement = "top", booktabs = TRUE, add.to.row = addtorow)
```


```{r, echo=FALSE, fig.align='center', fig.path= "Graphics"}

plot(tab.1[, 2:3], type="b", ylim=c(0, 40), ylab="", xaxt="no", xlab="", col="red", main="Figure 1: Education Outcomes: A Comparison of Scores at Intake and Discharge", cex.axis = 0.8, cex.main = 0.8, font.main = 1, family="serif")
par(new = T)
