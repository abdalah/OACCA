...
library(dplyr)
w2 = w %>% select(client_FID, agency_program_FID, cs_total.intake, cs_total.discharge, everything())

df.cs = subset(w2, cs_total.intake != "NA" & cs_total.discharge != "NA" & cs_total.intake != "NA" & cs_total.discharge != "NA", )


df.cs$cs_total.intake = as.numeric(as.character(df.cs$cs_total.intake))
df.cs$cs_total.discharge = as.numeric(as.character(df.cs$cs_total.discharge))
...
my.data2 = subset(odp, v3=="X" & length.of.stay.months >= 1 & client_num != 500000, )
...
library(dplyr)
w2b = w2 %>% select(client_FID, agency_program_FID, cs_total.intake, cs_total.discharge, everything())

df2.cs = subset(w2b, cs_total.intake != "NA" & cs_total.discharge != "NA" & cs_total.intake != "NA" & cs_total.discharge != "NA", )


df2.cs$cs_total.intake = as.numeric(as.character(df2.cs$cs_total.intake))
df2.cs$cs_total.discharge = as.numeric(as.character(df2.cs$cs_total.discharge))
