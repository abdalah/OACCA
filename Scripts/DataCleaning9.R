...
odp$court2 <- odp$court_involve
odp$court2[odp$court_involve == -2] <- -1

odp$court <- factor(odp$court2, levels = c(1, 0, -1), labels = c("Yes", "No", "Unknown"))


library(dplyr)
w_odp = odp %>% select(client_FID, agency_program_FID, start_date, end_date, follow_up_date, age.years, everything())


my.data = subset(w_odp, v3=="X" & length.of.stay.months >= 1 & client_num != 500000 & age.years >= 17, )


df <- my.data[!(duplicated(my.data[c("client_FID", "start_date", "end_date", "follow_up_date")]) | duplicated(my.data[c("client_FID", "start_date", "end_date", "follow_up_date")], fromLast = TRUE)), ]
