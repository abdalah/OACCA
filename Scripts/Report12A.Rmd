...
agglength2 <- df2[ df2$program == prog, ]
agglength3 <- df3[ df3$program == prog, ]
```
...
\begin{itemize}
\item `r length(agglength$client_FID)` Aggregate records available
\end{itemize}
...
\begin{itemize}
\item `r length(agglength3$client_FID)` Aggregate records available
\end{itemize}
