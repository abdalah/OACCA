...
\item `r length(prog$client_FID)` Agency records available
\end{itemize}
...
roles.ppA = mean(prog$p2a_total.intake, na.rm = T)
roles.pA = mean(prog$p2a_total.discharge, na.rm = T)
roles.dA = mean(prog$home.discharge, na.rm = T)
nobsA = length(prog$client_FID)

tab.1 = cbind(nobs, roles.pp, roles.p, roles.d, nobsA, roles.ppA, roles.pA, roles.dA)

addtorow <- list()
addtorow$pos <- list()
addtorow$pos[[1]] <- 0
addtorow$pos[[2]] <- 0
addtorow$command <- c(' \\multicolumn{4}{c}{Aggregate} &  \\multicolumn{4}{c}{Agency} \\\\\n', 'N & Prior to Placement & During Placement & Discharge & N & Prior to Placement & During Placement & Discharge \\\\\n')

print(xtable(tab.1, align="lrrrrrrrr", caption = "Restrictiveness of Living Levels Prior to Placement, During Placement and to which the Child will be discharged", digits=c(0, 0, 2, 2, 2, 0, 2, 2, 2)), type='latex', comment=FALSE, include.rownames=FALSE, include.colnames = F, caption.placement = "top", booktabs = TRUE, add.to.row = addtorow)
```


```{r, echo=FALSE, fig.align='center', fig.path= "Figure/plots"}

plot(tab.1[, 2:4], type="b", ylim=c(0, 20), ylab="", xaxt="no", xlab="", col="red", main="Figure 1: Restrictiveness of Living Levels", cex.axis = 0.8, cex.main = 0.8, font.main = 1, family="serif")
par(new = T)
lines(tab.1[,6:8], col = "blue", pch = 16)
...
prog <- subset(prog, !is.na(p2atotal_followup))
...
if(mean(progaggregate$p2atotal_followup, na.rm = T) != "NaN"){
  ...
  roles.dA = mean(prog$home.discharge, na.rm = T)
  roles.fA = mean(prog$p2atotal_followup, na.rm = T)  
  nobsA = length(prog$client_FID)
  
  tab.2 = cbind.data.frame(nobs, roles.d, roles.f, nobsA,roles.dA, roles.fA)
  tab.2[is.na(tab.2)] <- "NA"
  
  addtorow <- list()
  addtorow$pos <- list()
  addtorow$pos[[1]] <- 0
  addtorow$pos[[2]] <- 0
  addtorow$command <- c(' \\multicolumn{3}{c}{Aggregate} &  \\multicolumn{3}{c}{Agency} \\\\\n', 'N & Discharge & Follow-up & N & Discharge & Follow-up \\\\\n')
  
  print(xtable(tab.2, align="lrrrrrr", caption = "Restrictiveness of Living Levels at Discharge and at Follow-up", digits=c(0, 0, 2, 2, 0, 2, 2)), type='latex', comment=FALSE, include.rownames=FALSE, include.colnames = F, caption.placement = "top", booktabs = TRUE, add.to.row = addtorow)
...
```{r, echo=FALSE, fig.align='center', fig.path= "Figure/plots"}
if(mean(progaggregate$p2atotal_followup, na.rm = T) != "NaN"){
  tab.2 = cbind(nobs, roles.d, roles.f, nobsA,roles.dA, roles.fA)
  plot(tab.2[, 2:3], type="b", ylim=c(0, 20), ylab="", xaxt="no", xlab="", col="red", main="Figure 2: Restrictiveness of Living Levels", cex.axis = 0.8, cex.main = 0.8, font.main = 1, family="serif")
  par(new = T)
  lines(tab.2[,5:6], col = "blue", pch = 16)
  axis(1, at=1:2, labels=c("Discharge", "Follow-up"), col.axis="black", cex.lab = 0.75, las=1, cex.axis = .75)
  legend("topright", col = c("red", "blue"), pch = c(1, 1), cex = 0.75, legend = c("Aggregate", "Agency"))
}
```
