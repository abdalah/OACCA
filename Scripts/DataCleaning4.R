## Data Cleaning - Report 4 ##

odp <- read.csv("Data/placement_roles.csv", na.strings = "NA")

names(odp)

odp$start_date = odp$placement_start
odp$end_date = odp$placement_end
...
library(dplyr)
w_odp = odp %>% select(client_FID, start.date, end.date,everything())


my.data = subset(w_odp, length.of.stay.months >= 1 & client_num != 500000 & agency_FID != 1, )

df <- my.data[!(duplicated(my.data[c("client_FID","direction")]) | duplicated(my.data[c("client_FID","direction")], fromLast = TRUE)), ]
...
w <- reshape(df,
             timevar = "direction",
             idvar = c("client_FID", "start_date", "end_date"),
             direction = "wide")
...
df$p2atotal_followup = df$p2atotal_rolesfollow.discharge
df$program_loop <- paste(df$agencyID.intake, df$program.intake)

df <- df[ df$program_loop != "NA NA",]
