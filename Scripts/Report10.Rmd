...
if(length(agglength[,1]) > 0){
  ...
  barplot(tab.1plot, ylim=c(0, 100), beside = TRUE, ylab="Percent", xlab="Abuse/Neglect", col="gray", main="Figure 1: Safety During Placement", cex.axis = 0.8, cex.main = 0.8, font.main = 1, family="serif")
  #axis(1, at=1:2, labels=c("Intake", "Discharge"), col.axis="black", las=1)
...
agglength <- df2[ df2$program == program_name, ]

prog2 <- prog2[ !is.na(prog2$abuse), ]
agglength <- agglength[ !is.na(agglength$abuse), ]
```

\begin{itemize}
\item `r length(agglength$abuse)` Aggregate records available
\item `r length(prog2$abuse)` Agency records available
\end{itemize}


```{r, echo=FALSE}
# Table 2: Safety Post Discharge (number*, percent***, agency and aggregate*)

# Figure 2: Safety Post Discharge (bar graph**** of percent***, agency and aggregate*)
```
...
if(length(agglength[,1]) > 0){
  ...
  print(xtable(tab.1Main, align="llrrrr", caption = "Safety Post Discharge", digits = c(0, 0, 0, 2, 0, 2)),hline.after = c(0, -1, (length(tab.1Main[,1])-1), length(tab.1Main[,1])), type='latex', comment=FALSE, include.rownames=FALSE, include.colnames = F, caption.placement = "top", booktabs = TRUE, add.to.row = addtorow)
  } else{cat("Note: No useable records are available to generate the table and figure.")}
```
...
if(length(agglength[,1]) > 0){
  tab.1plot = cbind(as.numeric(tab.1b), as.numeric(tab.11b))
  colnames(tab.1plot) <- c("Aggregate", "Agency")
  barplot(tab.1plot, ylim=c(0, 100), beside = TRUE, ylab="Percent", xlab="Abuse/Neglect", col="gray", main="Figure 2: Safety Post Discharge", cex.axis = 0.8, cex.main = 0.8, font.main = 1, family="serif")
  #axis(1, at=1:2, labels=c("Intake", "Discharge"), col.axis="black", las=1)
} else{cat("Note: No useable records are available to generate the table and figure.")}
```
