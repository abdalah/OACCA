...
odp <- read.csv("Data/report1.csv", na.strings="NA")
...
odp$discharge <- factor(odp$nature_discharge, levels = c(1, 2, 3, 4, 5, -2), labels = c("Planned Discharge", "Administrative Discharge", "Withdrawal by Placing Agency Prior to Completion of Plan", "Self Discharge", "Court Removal", "No Answer"))
...
odp$diagnosis <- factor(odp$intake_diagnosis, labels=c("Adjustment Disorders", "All Other Diagnoses", "All Other Disorders Usually First Diagnosed in Infancy", "Anxiety Disorders", "Attention Deficit & Disruptive Behavior Disorders", "Mood Disorders"))
...
df = my.data[!duplicated(my.data$client_FID),]
