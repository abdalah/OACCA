...
my.data = subset(w_odp, v3=="X" & length.of.stay.months >= 1 & client_num != 500000 & abuse_neglect_v3 != "NA" & placement_30daysorless == 0 & end_date != "", )
...
df$abuse = factor(df$abuse_neglect_v3, levels=c(0, 1), labels = c("No", "Yes"))
...
df$program_loop <- paste(df$agency_FID, df$program)


odp <- read.csv("Data/followup_summary.csv", na.strings = "NA")
...
my.data = subset(w_odp, v3=="X" & length.of.stay.months >= 1 & client_num != 500000 & child_abuse != "NA" & placement_30daysorless == 0 & end_date != "", )

#df2 = my.data[!duplicated(my.data[, c("client_FID", "start_date", "end_date")]), ]

df2 <- my.data[!(duplicated(my.data[c("client_FID","start_date", "end_date")]) | duplicated(my.data[c("client_FID","start_date", "end_date")], fromLast = TRUE)), ]


df2$abuse = factor(df2$child_abuse, levels=c(0, 1), labels = c("No", "Yes"))
